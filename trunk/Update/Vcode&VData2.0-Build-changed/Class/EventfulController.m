...
{
    return [item isKindOfClass:[EventFolder class]];
}
...
{	//log or something to figure out which table column?
   /* if([item isKindOfClass:[EventFolder class]]) {
        return [item folderGroupName];
    }
...
/*- (int)numberOfRowsInTableView:(NSOutlineView *)aOutlineView{
	return [[doc eventFolders] count];  
}*/
